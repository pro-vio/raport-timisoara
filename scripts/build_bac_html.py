...
# Sinteza „Ce spune imaginea completă" se calculează, nu se scrie de mână.
comp = fil['compozitie']
REZ = fil['rezumat']
_pf = [fil['pe_ani'][str(a)][f] for a in YEARS for f in FILIERE if 'mediane' in fil['pe_ani'][str(a)][f]]
EPS_MIN, EPS_MAX = min(x['epsilon2'] for x in _pf), max(x['epsilon2'] for x in _pf)
KW_SEMN = sum(1 for x in _pf if x['p'] < 0.05)
KW_TOT = len(_pf)
DUNN_SEMN = [(a, f, d['pereche'], d['p_holm'])
             for a in YEARS for f in FILIERE
             for d in fil['pe_ani'][str(a)][f].get('dunn', []) if d['p_holm'] < 0.05]
def _dif_tm(f, a):
    m = fil['pe_ani'][str(a)][f].get('mediane')
    return None if not m else (m['CLUJ-NAPOCA'] - m['TIMIȘOARA'], m['IAȘI'] - m['TIMIȘOARA'])
CMP = {f: {'2017': _dif_tm(f, 2017), '2025': _dif_tm(f, 2025)} for f in FILIERE}
W_FIL = {f: [fr[f'{c}|{f}']['kendall_W'] for c in ('CLUJ-NAPOCA', 'IAȘI', 'TIMIȘOARA')
             if f'{c}|{f}' in fr] for f in FILIERE}
COMP_TEHN = {c: (min(comp[f'{c}|{a}']['tehnologica'] for a in YEARS),
                 max(comp[f'{c}|{a}']['tehnologica'] for a in YEARS))
             for c in ('CLUJ-NAPOCA', 'IAȘI', 'TIMIȘOARA')}
...
.chart .brut{stroke:var(--muted);stroke-width:1.6}
...
.leg svg{flex:none}
.tests{margin:0;padding-left:20px;font-size:14.5px;color:var(--ink)}
.tests li{margin-bottom:9px}
.comp td.num{font-variant-numeric:tabular-nums;text-align:right}
svg.schema{min-width:620px;height:auto;font-family:var(--sans)}
.schema .s-ci{stroke:var(--accent);stroke-width:2.5;opacity:.5;stroke-linecap:round}
.schema .s-pt{fill:var(--accent);stroke:var(--surface);stroke-width:1.5}
.schema .s-brut{stroke:var(--muted);stroke-width:2}
.schema .s-lead{stroke:var(--line);stroke-width:1}
.schema .s-lbl{fill:var(--ink);font-size:12.5px}
.schema .s-lbl2{fill:var(--muted);font-size:11.5px}
.schema .s-zone{fill:var(--chip)}
...
<div class="card">
  <div class="card-h">Compoziția orașelor</div>
  <p class="card-i">Ponderea candidaților pe filieră, în fiecare oraș. E singura cifră din raport unde orașul apare întreg, fiindcă e o afirmație despre structura lui, nu o comparație de performanță.</p>
  <div id="v-comp"></div>
  <p class="note">Explicația comodă a decalajului Timișoarei ar fi că are mai multe licee tehnologice. Cluj chiar are mai puține — {pct(COMP_TEHN['CLUJ-NAPOCA'][0])}–{pct(COMP_TEHN['CLUJ-NAPOCA'][1])} față de {pct(COMP_TEHN['TIMIȘOARA'][0])}–{pct(COMP_TEHN['TIMIȘOARA'][1])} la Timișoara. Dar Iași are {pct(COMP_TEHN['IAȘI'][0])}–{pct(COMP_TEHN['IAȘI'][1])}, adică aceeași compoziție ca Timișoara, și stă totuși deasupra ei în ambele filiere, în toți anii. Deci compoziția explică o parte din decalajul față de Cluj, dar nu poate fi toată explicația.</p>
</div>

<div class="card">
  <div class="card-h">Testele efectuate</div>
  <ul class="tests">
    <li><strong>Kruskal-Wallis</strong> pe fiecare an și fiecare filieră separat, cu rangurile calculate doar în interiorul filierei. Unitatea de observație = celula liceu×filieră; observațiile sunt medianele celulelor, grupate pe oraș. Prag: minim 8 candidați pe celulă, minim 3 licee pe oraș ca filiera să intre în test într-un an.</li>
    <li><strong>Dunn</strong> post-hoc, pe o familie <em>pre-specificată</em>: aceeași filieră, între orașe — 3 filiere × 3 perechi de orașe = 9 comparații pe an, cu corecție <strong>Holm</strong> peste ele. Familia e declarată dinainte fiindcă Dunn pe toate perechile ar da 36, iar Holm ar tăia puterea plătind pentru comparații fără interes (teoreticele din Timișoara față de tehnologicele din Cluj nu spun nimic).</li>
    <li><strong>Friedman</strong> pe fiecare dintre cele {FR_N} entități oraș×filieră: blocuri = celulele liceu×filieră prezente în toți cei nouă ani, tratament = anul, cu anii ordonați după mediana celulei (graficul de mai sus).</li>
  </ul>
</div>

<div class="card">
  <div class="card-h">Ce spune imaginea completă</div>
  <div class="prose" style="max-width:none;font-size:15.5px">
  <p><strong>Anii nu sunt interschimbabili, dar filierele nu se mișcă la fel.</strong> Friedman respinge omogenitatea temporală în {FR_REJ} din cele {FR_N} entități{FR_EXC_FRAZA}. Aici se oprește însă analogia cu Evaluarea Națională, unde cele trei orașe urcau și coborau împreună și de aceea efectul putea fi numit național. La bacalaureat, asta rămâne adevărat <em>între orașe, în interiorul aceleiași filiere</em> — dar nu între filiere: la Timișoara cel mai bun an e {AN_TOP['teoretica']} la teoretic, {AN_TOP['tehnologica']} la tehnologic și {AN_TOP['vocationala']} la vocațional, iar concordanța dintre licee (Kendall W) sare de la {d(min(W_FIL['teoretica']))} la teoretic până la {d(max(W_FIL['tehnologica']))} la tehnologic. Cele trei filiere trăiesc examene diferite.</p>
  <p><strong>Între orașe, diferențele sunt mici și rareori semnificative.</strong> Din cele {KW_TOT} de teste (fiecare an × fiecare filieră), {KW_SEMN} ies sub 0,05, cu mărimi de efect între ε²={d(EPS_MIN)} și ε²={d(EPS_MAX)}. Din cele 81 de comparații pre-specificate între perechi de orașe, doar {len(DUNN_SEMN)} trec pragul după corecția Holm. Deci varianța structurală între orașe există, dar e modestă.</p>
  <p><strong>Direcția, în schimb, e neverosimil de constantă.</strong> Timișoara e ultima din trei la teoretic în {REZ['teoretica']['tm_ultima']} ani din {REZ['teoretica']['ani']} și la tehnologic în {REZ['tehnologica']['tm_ultima']} din {REZ['tehnologica']['ani']}. La vocațional, doar în {REZ['vocationala']['tm_ultima']} din {REZ['vocationala']['ani']} — acolo Timișoara nu are o problemă. Constanța asta nu se poate transforma în valoare p, fiindcă anii împart aceleași licee și nu sunt observații independente; dar nici nu e ceva care să se explice prin hazard.</p>
  <p><strong>Decalajul se lărgește.</strong> În 2017, teoreticele timișorene erau sub cele clujene cu {d(CMP['teoretica']['2017'][0])} puncte de mediană; în 2025, cu {d(CMP['teoretica']['2025'][0])}. La tehnologic, distanța trece de la {d(CMP['tehnologica']['2017'][0])} la {d(CMP['tehnologica']['2025'][0])}. Cele patru comparații semnificative din nouă ani sunt concentrate la capătul recent: {', '.join(f"{f[:5]} vs {pe.split(' vs ')[0]} în {a}" for a, f, pe, _ in sorted(DUNN_SEMN))}.</p>
  <p><strong>Consecința metodologică.</strong> Interacțiunea filieră × an e reală, deci nicio statistică nu se agregă între filiere — nici clasamentul, nici testul, nici distribuția, nici măcar în interiorul aceluiași liceu. Mulțimea de referință e întotdeauna celula oraș×filieră. Iar variația între ani fiind un efect comun tuturor liceelor unei filiere, ar contamina orice comparație multi-an pe valori brute: de aceea fiecare an se citește separat, iar evoluția în timp se privește pe reziduu.</p>
  </div>
</div>
</section>
...
<div class="card">
  <div class="card-h">Cum se citește graficul</div>
  <p class="card-i">Punctul e mediana ajustată; bara e intervalul în care datele o pot localiza. Când barele a două licee se suprapun, diferența dintre ele nu se susține — oricât de departe ar fi punctele.</p>
  <div class="scroll"><svg viewBox="0 0 660 218" class="schema" role="img" aria-label="Schemă: punctul e mediana ajustată, bara e intervalul de 95%. Când barele a două licee se suprapun, diferența nu e semnificativă; când sunt separate, diferența se susține.">
    <line x1="150" y1="34" x2="410" y2="34" class="s-ci"/>
    <line x1="250" y1="26" x2="250" y2="42" class="s-brut"/>
    <circle cx="320" cy="34" r="5.5" class="s-pt"/>
    <line x1="320" y1="12" x2="320" y2="27" class="s-lead"/>
    <text x="320" y="9" text-anchor="middle" class="s-lbl">mediana ajustată</text>
    <line x1="150" y1="41" x2="150" y2="58" class="s-lead"/>
    <text x="150" y="71" text-anchor="middle" class="s-lbl2">interval 95%</text>
    <line x1="250" y1="43" x2="250" y2="58" class="s-lead"/>
    <text x="250" y="71" text-anchor="middle" class="s-lbl2">mediana brută</text>
    <rect x="250" y="98" width="90" height="54" class="s-zone"/>
    <text x="295" y="93" text-anchor="middle" class="s-lbl2">se suprapun</text>
    <text x="16" y="112" class="s-lbl">Liceul A</text>
    <line x1="170" y1="108" x2="340" y2="108" class="s-ci"/><circle cx="255" cy="108" r="5.5" class="s-pt"/>
    <text x="16" y="146" class="s-lbl">Liceul B</text>
    <line x1="250" y1="142" x2="420" y2="142" class="s-ci"/><circle cx="335" cy="142" r="5.5" class="s-pt"/>
    <text x="440" y="118" class="s-lbl">diferența</text><text x="440" y="134" class="s-lbl">NU se susține</text>
    <text x="16" y="184" class="s-lbl">Liceul C</text>
    <line x1="170" y1="180" x2="275" y2="180" class="s-ci"/><circle cx="222" cy="180" r="5.5" class="s-pt"/>
    <text x="16" y="210" class="s-lbl">Liceul D</text>
    <line x1="345" y1="206" x2="450" y2="206" class="s-ci"/><circle cx="397" cy="206" r="5.5" class="s-pt"/>
    <text x="470" y="194" class="s-lbl">diferența</text><text x="470" y="210" class="s-lbl">se susține</text>
  </svg></div>
</div>

<div class="card">
  <div class="card-h">Notă metodologică</div>
  <div class="prose" style="max-width:none;font-size:15.5px">
  <p>Pentru fiecare celulă liceu×filieră: <strong>mediana</strong> mediilor candidaților ei, cu <strong>eroarea standard prin bootstrap</strong> — 2.000 de reeșantionări cu înlocuire din candidații celulei, incluzându-i pe cei fără rezultat. Mediana e trasă apoi spre ancora lumii ei proporțional cu incertitudinea: liceele cu puțini candidați se apropie de ancoră și primesc interval larg, cele mari rămân aproape neatinse. Un clasament neajustat le-ar da celor mici un loc pe care datele nu-l susțin.</p>
  <p><strong>Ancora e a celulei oraș×filieră×an, nu a orașului.</strong> Un liceu tehnologic tras spre media tuturor liceelor ar fi penalizat fiindcă e tehnologic, ceea ce nu e o informație despre el. Nici anii nu se amestecă: Friedman a arătat că nu sunt interschimbabili, deci și ancora, și dispersia priorului se estimează separat pe fiecare an.</p>
  <p>Ancora e <strong>media</strong> medianelor liceelor din celulă, nu mediana lor. E o alegere impusă de formula de shrinkage, care descompune dispersia totală (τ² plus zgomotul de eșantionare) matematic față de medie. Soluția e deci intenționat hibridă: la nivelul candidaților, unde asimetria contează, statistica rămâne strict mediana; la nivelul agregării peste cele 7–18 licee ale unei celule, unde contează doar tendința centrală a unui set mult mai omogen, se folosește media — singura care face formula corectă.</p>
  <p>τ² e estimat prin metoda momentelor: dispersia observată între licee minus cea explicabilă prin zgomot, tăiată la zero. Când iese mare față de zgomot — cum se întâmplă aici — ponderea w se apropie de 1, iar ajustarea aproape nu mișcă nimic. Asta nu e un defect al metodei, ci un rezultat: liceele chiar diferă între ele mai mult decât ne înșeală eșantionul.</p>
  </div>
</div>
</section>
</div>
...
function renderTM(){
  ...
  S.forEach((s,i)=>{
    ...
    g+=`<g class="row">
      ...
      <line class="brut" x1="${X(s.br)}" y1="${y-5}" x2="${X(s.br)}" y2="${y+5}"/>
      <circle class="pt" cx="${X(s.sh)}" cy="${y}" r="5" fill="${col}"/></g>`;
  });
  g+='</svg>';
  h+=`<div class="card"><div class="card-h">Mediana ajustată și intervalul ei</div>
    <p class="card-i">Punctul e mediana după ajustare; liniuța verticală e mediana brută, dinainte de ajustare; bara e intervalul de 95% în care datele o pot localiza. Liceele ale căror bare se suprapun nu pot fi despărțite — vezi schema de sub tabel. Sortare după mediana ajustată.</p>
    <div class="scroll">${g}</div></div>`;
...
// compoziția: singurul loc unde orașul apare întreg — e o afirmație despre structura lui
(function renderComp(){
  const C=['CLUJ-NAPOCA','IAȘI','TIMIȘOARA'], NC={'CLUJ-NAPOCA':'Cluj-Napoca','IAȘI':'Iași','TIMIȘOARA':'Timișoara'};
  let h='<div class="scroll"><table class="comp"><thead><tr><th class="l">An</th>';
  C.forEach(c=>{h+='<th colspan="3" style="text-align:center">'+NC[c]+'</th>';});
  h+='</tr><tr><th class="l"></th>'+C.map(()=>'<th>teor.</th><th>tehn.</th><th>voc.</th>').join('')+'</tr></thead><tbody>';
  D.ani.forEach(a=>{ h+='<tr><td class="l">'+a+'</td>';
    C.forEach(c=>{ const k=D.comp[c+'|'+a];
      D.filiere.forEach(f=>{ h+='<td class="num">'+(k?Math.round(k[f]*100)+'%':'—')+'</td>'; }); });
    h+='</tr>'; });
  h+='</tbody></table></div>';
  document.getElementById('v-comp').innerHTML=h;
})();
