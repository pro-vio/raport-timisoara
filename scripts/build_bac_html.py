...
<header>
  ...
  <p class="sub">Sesiunea de vară · promoția curentă · trei filiere tratate separat</p>
</header>

<nav role="tablist">
  <button class="tab" role="tab" id="t-date" aria-controls="p-date" aria-selected="true">Datele și definițiile</button>
  <button class="tab" role="tab" id="t-orase" aria-controls="p-orase" aria-selected="false">Varianța structurală: trei orașe</button>
...
<p>Anii nu se adună. Testul Friedman, pe liceele prezente în toți cei nouă ani, respinge ipoteza că anii ar fi schimbabili între ei, în toate cele trei orașe: cea mai mare valoare p din cele trei e {putere(P_MAX_FR)}, iar coeficientul de concordanță Kendall stă între {d(W_MIN)} și {d(W_MAX)}. Tiparul e consecvent — 2018 e cel mai slab an aproape pretutindeni, 2024 cel mai bun. De aceea fiecare an se citește separat.</p>
